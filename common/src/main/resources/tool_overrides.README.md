...
`config/playerengine/tool_overrides.json` on first launch.
...
|------|----------------|-------------|
| `config/playerengine/tool_overrides.json` | Server operator / world host | Always (global) |
...
Kill switches (configurable in `config/playerengine/config.json`):
