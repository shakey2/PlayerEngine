 ...
 *              LivingEntity.eat() which shrinks the held stack by 1. The task's only job here
 *              is to call hungerManager().eat(foodItem) to refill the bot's hunger/saturation.
 *              Do NOT also call removeItem/shrink — that would double-consume the item.
    ...
    protected Task onTick() {
        ...
        switch (this.state) {
            ...
            case BEGIN_USE: {
                ...
                // getUseDuration() on ItemStack delegates to Item.getUseDuration(stack) — no-param call on ItemStack is the 1.20.1 pattern.
                int useDuration = inHand.isEmpty() ? 32 : inHand.getUseDuration();
            ...
            case WAIT: {
                ...
                if (!entity.isUsingItem()) {
                    ...
                    // which already called food.shrink(1)). Move to COMPLETE to refill hunger.
