...
public final class SignTextFormatting {
    ...
    public static String componentToJson(Component c, HolderLookup.Provider registries) {
     ...
     */
    public static Component parseLineToComponent(String raw, HolderLookup.Provider registries) {
        ...
        if (t.startsWith("{") || t.startsWith("[")) {
            try {
                ...
                MutableComponent c = Component.Serializer.fromJson(json, registries);
    ...
    public static Component[] fourLinesFromStrings(String[] lines, HolderLookup.Provider registries) {
        Component[] out = new Component[4];
        for (int i = 0; i < 4; i++) {
            String line = i < lines.length ? lines[i] : "";
            out[i] = parseLineToComponent(normalizeAiLineSlotPrefix(line), registries);
        }
        return out;
    }
