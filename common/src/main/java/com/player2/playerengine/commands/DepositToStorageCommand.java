 ...
 * bot's own inventory into the container at the given position (Part C4.5, WS4).
 *
 * <p>This is the PRECISE command: the legacy {@code deposit} keeps its bulk store-everything
 * behavior (and its non-gear keep rules), which deliberately do NOT apply here. The items tail
 * uses the {@link StorageItemArgs} grammar (Decision 11); an omitted count deposits all the bot
 * carries of that item (best-effort per entry). Explicit counts are atomic across the whole
 * entry list: if any explicit entry fails validation against the live 54-slot-aware container
 * view, nothing in the command moves — including its omitted-count entries — and the error
 * enumerates every failing entry with its exact live number.
 *
 * <p>{@code call()} only parses args and runs the travel-cap PRECHECK; the session logic lives
 * in {@link BoundedContainerTransferTask} (validation view == transfer view, Decision 4) and the
 * typed outcome is mapped to the finish paths inside the {@code runUserTask} callback. Every
 * failure and degradation reaches the model, the player
 * ({@code reportAgenticProgress(msg, true)}), and the server log — Decision 5.
 */
public class DepositToStorageCommand extends Command {

    public DepositToStorageCommand() throws CommandException {
        super(
                "deposit_to_storage",
                "deposit_to_storage <x> <y> <z> <items...>. Put a precise selection of your items into"
                        + " the container at x y z (chest/double chest/barrel/shulker). items: entries"
                        + " separated by \",\", each \"<item>\" or \"<item> <count>\"; omit the count to"
                        + " deposit all you carry. Example: deposit_to_storage 120 64 -33 dirt 64,"
                        + " oak_planks. Explicit counts are atomic: if any cannot fit, nothing moves and"
                        + " the error names the live room. Use deposit for bulk store-everything behavior.",
    ...
    protected void call(PlayerEngineController mod, ArgParser parser) throws CommandException {
        ...
        BoundedContainerTransferTask task = new BoundedContainerTransferTask(
                ScanReportFormatter.TransferDirection.DEPOSIT, pos, parsed.queries());
    ...
    /** Human item list for the player chat line, e.g. {@code "64 dirt, 30 oak planks"}. */
