 ...
 * withdrawal: take exactly {@code count} items out of ONE container slot (deep-scan indexing;
 * double chests are one 54-slot space, 0-53) into the bot's inventory, optionally into a
 * specific bot main slot 0-35 (omitted = auto-place) — Part C4.5, WS4.
 *
 ...
 * paths inside the {@code runUserTask} callback. Every failure reaches the model
 * ({@code StorageAccessCode}-token text), the player (one
 * {@code reportAgenticProgress(msg, true)} line), and the server log (structured
 * {@code storage-tx} one-liner) — Decision 5.
 */
public class WithdrawStorageSlotCommand extends Command {

    public WithdrawStorageSlotCommand() throws CommandException {
        super(
                "withdraw_storage_slot",
                "withdraw_storage_slot <x> <y> <z> <containerSlot> <count> [botSlot]. Take exactly"
                        + " count items from one slot of the container at x y z (slot indices match"
                        + " deep scan_storage output; a double chest is one 0-53 space). Use the slot's"
                        + " full count to take everything in it. botSlot (0-35) targets a specific bot"
                        + " inventory slot; omit it to auto-place."
                        + " Example: withdraw_storage_slot 120 64 -33 2 40 7.",
                new Arg<>(Integer.class, "x"),
                new Arg<>(Integer.class, "y"),
                new Arg<>(Integer.class, "z"),
                new Arg<>(Integer.class, "containerSlot"),
                new Arg<>(Integer.class, "count"),
                new Arg<>(Integer.class, "botSlot", SlotPreciseTransactionTask.SLOT_AUTO, 5, false));
    }
    ...
    protected void call(PlayerEngineController mod, ArgParser parser) throws CommandException {
        ...
        if (u == null || u.length < 5) {
            failNow(mod, null, StorageAccessCode.INVALID_ARGUMENT,
                    "expected <x> <y> <z> <containerSlot> <count> [botSlot]; usage: "
        ...
        int containerSlot;
        int count;
        int botSlot = SlotPreciseTransactionTask.SLOT_AUTO;
        try {
            x = Integer.parseInt(u[0].trim());
            y = Integer.parseInt(u[1].trim());
            z = Integer.parseInt(u[2].trim());
            containerSlot = Integer.parseInt(u[3].trim());
            count = Integer.parseInt(u[4].trim());
            if (u.length > 5) {
                botSlot = Integer.parseInt(u[5].trim());
            }
        ...
        String reqSummary = "slot:" + containerSlot + ",count:" + count
                + (botSlot != SlotPreciseTransactionTask.SLOT_AUTO ? ",botSlot:" + botSlot : "");
        ...
        if (botSlot != SlotPreciseTransactionTask.SLOT_AUTO
                && (botSlot < 0 || botSlot > SlotPreciseTransactionTask.MAX_BOT_SLOT)) {
            failNow(mod, pos, StorageAccessCode.INVALID_SLOT,
        ...
        SlotPreciseTransactionTask task = new SlotPreciseTransactionTask(
                ScanReportFormatter.TransferDirection.WITHDRAW, pos, containerSlot, botSlot, count);
        mod.runUserTask(task, () -> handleOutcome(mod, pos, reqSummary, requestedCount, task));
    }

    /** Maps the task's typed outcome to the finish paths (runs inside the task callback). */
    private void handleOutcome(
            PlayerEngineController mod, BlockPos pos, String reqSummary, int requestedCount,
            SlotPreciseTransactionTask task) {
        if (task.outcome() == SlotPreciseTransactionTask.Outcome.COMPLETED) {
            ...
            if (task.movedCount() < requestedCount) {
                // Auto-place packing edge: fewer landed than requested — the receipt (with the
                // actual moved count) rides the note so the model sees the truth.
